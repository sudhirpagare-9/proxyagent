...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] [NIST-CLOUD-SECURE] %(message)s")
...
# Cloud Database Configuration (Supabase, Neon, AWS RDS PostgreSQL or SQLite fallback)
...
def init_db():
    try:
        ...
        cursor.close()
        conn.close()
        db_type = "Cloud PostgreSQL (Supabase / External)" if DATABASE_URL else "Local SQLite WAL"
...
class HardenedSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if len(self.ip_records[client_ip]) >= self.rate_limit:
            return JSONResponse(status_code=429, content={"error": "Security Block: Rate limit exceeded."})
...
def database_info():
    db_type = "Cloud PostgreSQL (Supabase / External)" if DATABASE_URL else "SQLite (Local Persistent WAL)"
    db_path = DATABASE_URL.split("@")[-1] if DATABASE_URL else os.path.abspath(DB_FILE)
    return {
        "database_type": db_type,
        "storage_location": db_path,
        "isolation_mode": "Cloud Multi-Tenant Row-Level Partitioning with NIST Encryption",
...
async def register_client(request: Request):
    ...
    if not hw_id or not re.match(r'^[A-Z0-9\-]{8,64}$', hw_id):
        raise HTTPException(status_code=400, detail="Exploit Prevention: Invalid hardware identifier format.")
    ...
    geo_info = {"country": "India", "city": "Chandrapur", "region": "Maharashtra", "isp": "Cloud Enterprise Node"}
    try:
        if real_ip not in ["127.0.0.1", "localhost", "0.0.0.0"]:
            async with httpx.AsyncClient() as client:
                geo_resp = await client.get(f"https://ipapi.co/{real_ip}/json/", timeout=2.5)
...
def get_tenant_data(hw_id: str):
    ...
    try:
        if DATABASE_URL:
            cursor.execute("SELECT status, subscription_tier, balance_tokens, api_key, metadata FROM clients WHERE hw_id = %s", (hw_id,))
        else:
            cursor.execute("SELECT status, subscription_tier, balance_tokens, api_key, metadata FROM clients WHERE hw_id = ?", (hw_id,))
        row = cursor.fetchone()
    finally:
        cursor.close()
        conn.close()
...
async def log_traffic(request: Request):
    ...
    try:
        ...
        if not row or row[0] != "APPROVED":
            raise HTTPException(status_code=402, detail="Access denied: Node unapproved or unregistered")
...
async def openai_compatible_chat_completions(request: Request):
    ...
    try:
        if api_key:
            if DATABASE_URL:
                cursor.execute("SELECT hw_id, status, balance_tokens FROM clients WHERE api_key = %s", (api_key,))
            else:
                cursor.execute("SELECT hw_id, status, balance_tokens FROM clients WHERE api_key = ?", (api_key,))
        elif hw_id_header:
            if DATABASE_URL:
                cursor.execute("SELECT hw_id, status, balance_tokens FROM clients WHERE hw_id = %s", (hw_id_header,))
            else:
                cursor.execute("SELECT hw_id, status, balance_tokens FROM clients WHERE hw_id = ?", (hw_id_header,))
        else:
            raise HTTPException(status_code=401, detail="Authentication required: Provide Bearer API Key or X-HW-ID header.")
    ...
    if status_val != "APPROVED":
        raise HTTPException(status_code=402, detail="Tenant node pending approval or denied in Enterprise Control Plane.")
    ...
    try:
        if DATABASE_URL:
            cursor.execute("UPDATE clients SET balance_tokens = %s WHERE hw_id = %s", (new_balance, hw_id))
            cursor.execute("INSERT INTO traffic_logs (hw_id, payload_json) VALUES (%s, %s)", 
                           (hw_id, json.dumps({
...
async def proxy_messages(request: Request):
        ...
        if provider == "Gemini" and gemini_key:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    candidate = ai_data.get("candidates", [{}])[0]
                    text_resp = candidate.get("content", {}).get("parts", [{}])[0].get("text", "No response")
                    usage = ai_data.get("usageMetadata", {"promptTokenCount": len(sanitized_prompt)//4 + 5, "candidatesTokenCount": 50})
                    return {
    ...
    return {
        ...
        "usage": {"input_tokens": max(10, len(sanitized_prompt.split()) * 2), "output_tokens": 50},
        "timestamp_utc": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    }
...
def dashboard_data():
    ...
    clients = [ {**json.loads(r[5] or "{}"), "hw_id": r[0], "status": r[1], "subscription_tier": r[2], "balance_tokens": r[3], "created_at": str(r[4]), "api_key": r[6]} for r in client_rows ]
...
async def gdpr_erase_data(request: Request):
    ...
    try:
        ...
        cursor.close()
        conn.close()
    return {"status": "success", "message": f"Tenant {hw_id} and audit logs permanently scrubbed under GDPR Article 17."}
...
async def client_action(request: Request):
    ...
    try:
        if DATABASE_URL:
            if action == "approve": cursor.execute("UPDATE clients SET status = 'APPROVED' WHERE hw_id = %s", (hw_id,))
            elif action == "deny": cursor.execute("UPDATE clients SET status = 'DENIED' WHERE hw_id = %s", (hw_id,))
            elif action == "tier": cursor.execute("UPDATE clients SET subscription_tier = %s WHERE hw_id = %s", (tier, hw_id))
            elif action == "topup": cursor.execute("UPDATE clients SET balance_tokens = balance_tokens + %s WHERE hw_id = %s", (amount, hw_id))
            elif action == "delete":
                cursor.execute("DELETE FROM clients WHERE hw_id = %s", (hw_id,))
                cursor.execute("DELETE FROM traffic_logs WHERE hw_id = %s", (hw_id,))
            conn.commit()
        else:
            if action == "approve": cursor.execute("UPDATE clients SET status = 'APPROVED' WHERE hw_id = ?", (hw_id,))
            elif action == "deny": cursor.execute("UPDATE clients SET status = 'DENIED' WHERE hw_id = ?", (hw_id,))
            elif action == "tier": cursor.execute("UPDATE clients SET subscription_tier = ? WHERE hw_id = ?", (tier, hw_id))
            elif action == "topup": cursor.execute("UPDATE clients SET balance_tokens = balance_tokens + ? WHERE hw_id = ?", (amount, hw_id))
            elif action == "delete":
                cursor.execute("DELETE FROM clients WHERE hw_id = ?", (hw_id,))
                cursor.execute("DELETE FROM traffic_logs WHERE hw_id = ?", (hw_id,))
            conn.commit()
...
def export_audit_report():
    ...
    output = io.StringIO()
    output.write("HardwareID,Provider,Model,ThinkingLevel,InputTokens,OutputTokens,TimestampUTC\n")
    for r in rows:
        p = json.loads(r[1] or "{}")
        output.write(f'"{r[0]}","{p.get("provider","N/A")}","{p.get("model","N/A")}","{p.get("thinking_level","Standard")}",{p.get("i",0)},{p.get("o",0)},"{p.get("timestamp_utc","N/A")}"\n')
...
<body class="min-h-screen p-6 flex flex-col">
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6 flex flex-col md:flex-row items-center justify-between gap-3 shadow-md font-mono text-xs">
        <div class="flex items-center gap-3">
            ...
            <div>
                <span class="text-gray-400">Target Storage:</span> <span id="db-path-display" class="text-emerald-400 font-bold">Connecting to cloud DB...</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function switchTab(tab) {
            if (tab === 'browser') {
                ...
                document.getElementById('btn-tab-browser').className = 'px-3 py-1 bg-gray-800 text-gray-400 rounded hover:text-white font-medium';
